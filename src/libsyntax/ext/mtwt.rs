...
fn apply_mark_internal(m: Mrk, ctxt: SyntaxContext, table: &SCTable) -> SyntaxContext {
    let key = (ctxt, m);
    * table.mark_memo.borrow_mut().entry(&key).get()
        .unwrap_or_else(|vacant_entry| vacant_entry.insert(idx_push(&mut *table.table.borrow_mut(), Mark(m, ctxt))))
}
...
fn apply_rename_internal(id: Ident,
    ...
    * table.rename_memo.borrow_mut().entry(&key).get()
        .unwrap_or_else(|vacant_entry| vacant_entry.insert(idx_push(&mut *table.table.borrow_mut(), Rename(id, to, ctxt))))
}
